...
export async function refreshToken(refresh_token: string){
    ...
    try{
        response = await fetch(
            AUTH_CONSTANTS.PATHS.REFRESH_TOKEN_URL,
            {   
                method: "POST",
                headers: {
                    "Content-Type": "application/x-www-form-urlencoded",
                },
                body: new URLSearchParams({grant_type: "refresh_token", scope: AUTH_CONSTANTS.SCOPE, client_id: AUTH_CONSTANTS.CLIENT_ID, client_secret: AUTH_CONSTANTS.CLIENT_SECRET, refresh_token: refresh_token})
            }
        )
        data = await response.json();
...
export async function logout(refresh_token: string){
    try{
        const response = await fetch(
            AUTH_CONSTANTS.PATHS.LOGOUT_URL,
            {   
                method: "POST",
                headers: {
                    "Content-Type": "application/x-www-form-urlencoded",
                },
                body: new URLSearchParams({ client_id: AUTH_CONSTANTS.CLIENT_ID, client_secret: AUTH_CONSTANTS.CLIENT_SECRET, refresh_token: refresh_token})
            }
        )
